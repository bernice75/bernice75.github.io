...
2024.10 ~ 2025.03 개발 및 개선을 진행했으며, 이후 2026.01까지 운영 환경에서 안정성을 관찰하고 검증했습니다.
...
모바일 POS에서는 NICE 결제 승인, 앱 내부 처리, 서버 동기화가 각각 다른 시점에 실패할 수 있어, 단계별 상태와 복구 흐름을 구분할 필요가 있었습니다.

#### 해결

결제 상태 설계에서는 하나의 성공/실패 상태 대신 결제 lifecycle을 ready → pending → paid → confirmed로 분리했습니다.
...
복구 흐름에서는 NICE 승인이 성공했지만 승인정보 저장에 실패한 경우, 결제를 완료 상태로 전환하지 않고 NICE 승인 이력을 확인하거나 취소할 수 있는 상태를 유지합니다.
...
재전송의 멱등성을 확보하기 위해 서버 반영에는 날짜 + 영수증번호를 기준으로 Upsert를 적용했습니다. 서버에는 정상 반영됐지만 클라이언트가 응답을 받지 못한 경우 동일 데이터를 다시 전송하더라도 새로운 row를 생성하지 않도록 해 재전송에 따른 중복 데이터 생성을 방지했습니다.
...
결제 상태와 동기화 상태를 분리하고, 미전송 데이터 복구와 중복 저장 방지를 위한 처리 구조를 구현했습니다. 현재 개발 진행 중으로 운영 성과는 아직 검증 전입니다.
...
FCM 발송 성공만으로는 앱의 실제 수신 여부를 알 수 없어, 필수 알림의 미수신을 확인하고 대체 알림과 재전송으로 이어지는 흐름이 필요했습니다.
...
특정 키오스크 한 대에서 필수 오류 알림이 반복되는 현상을 운영 데이터로 확인하고, 중복 알림을 방지하기 위해 cooldown을 적용했습니다.

#### 성과

발신 시스템 → FCM → 수신 앱 → 앱 수신 확인까지 전체 전달 lifecycle을 관찰할 수 있는 구조를 구축했습니다. 필수 알림 미수신 시 관리자 대체 알림과 구독 복구 후 재전송을 처리하는 중앙 Push Server를 운영하고 있습니다.
...
기존에는 서버에 SSH로 접속해 애플리케이션을 중지하고 JAR를 교체한 뒤 다시 실행하는 방식으로 배포했습니다. 문제가 발생하면 복구 과정이 사용자에게 그대로 노출될 수 있었습니다. 수동 배포의 인적 오류 위험과 신속한 롤백의 어려움을 줄이기 위해 배포 자동화 환경 구축을 제안하고 주도했습니다.

#### 해결

아이디어와 필수 기능을 직접 정리한 뒤, AI를 계획 초안 작성·설계 검토·구현에 활용했습니다. 계획 작성과 검토 역할을 서로 다른 AI에 맡겨 동시 배포 충돌, 실패 상태, 성공 판정 기준 등의 설계 허점을 검토하고 구현 계획을 구체화했습니다.
...
구현 이후에는 실제 배포 로그를 확인해 내부 Health Check, Proxy 전환, 외부 Health Check, 이전 프로세스 정리가 설계한 순서대로 수행되는지 검증했습니다.
...
기존 키오스크의 유지보수 한계를 개선하기 위해 신규 환경 구축을 제안하고 주도했습니다. 또한 장애 발생 시 고객 문의 이후 관리자가 상황을 인지하는 경우가 많아, 오류 감지와 원격 진단을 통한 운영 개선이 필요했습니다.
...
신규 키오스크에서는 상품·현금 모듈·카드 결제·결제 데이터 생성·영수증 출력 등의 오류를 감지해 Push Server로 전달하도록 했습니다.
...
구축은 2026.08에 완료했으며 현재는 신규 기능 추가와 유지보수를 진행하고 있습니다.
...
입사 직후 기존 운영 환경과 주문 업무 흐름을 파악해 3개월 내 MVP를 구현했습니다. 이후 3개월간 세부 조정과 추가 개발을 거쳐 2024.02 정식 출시했습니다.

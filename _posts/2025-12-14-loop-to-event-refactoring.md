...
"느리다"는 말 한 마디에서 시작됐습니다.

현업에서 올라온 문의였고, 일부 요청은 최대 7초 가까이 응답이 지연되고 있었습니다. <br>
24년 초에 구축 이후 문제 없이 써오던 서비스였는데... 갑작스러운 병목을 맞았습니다.<br>

이 글은 `for` 루프 안에서 외부 API를 즉시 호출하던 구조가 왜 한계에 부딪혔는지, 그리고 어떤 기준으로 이벤트 방식으로 전환을 결정했는지를 정리한 기록입니다. <br>
대규모 아키텍처 전환기라기보다는, "지금은 동작하지만 계속 이 구조로 가도 괜찮을까?"를 고민하던 끝에 내린 현실적인 선택에 가깝습니다.

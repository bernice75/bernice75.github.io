...
3일에 한 번씩 데이터베이스 커넥션이 고갈되는 문제로 추석 연휴까지 반납하게 되었습니다...<br>
POS/키오스크 기기에서 매출 데이터를 동기화하는 서버였는데, 전 담당자가 문서 없이 퇴사한 상태라 코드를 처음부터 읽어야 하는 어려움이 있었고,<br>
WebFlux + Netty 기반이라 Spring Boot/Java만 다뤄온 입장에서는 생소한 영역이었습니다.


이 글은 비동기 서버를 처음 마주한 입문자가 커넥션 고갈 문제를 어떻게 분석하고 해결했는지, 그 과정을 순서대로 정리한 기록입니다.<br>
비동기 환경 트러블슈팅을 처음 접하는 분들께 작은 힌트가 되길 바랍니다.
...
다음 개선을 이해하려면 리액터의 `subscribe` 개념을 먼저 짚고 넘어가는 게 좋습니다.
